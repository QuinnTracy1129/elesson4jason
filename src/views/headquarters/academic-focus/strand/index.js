...
export default class extends Component {
    constructor() {
        ...
        this.state = {
            entity: 'forbidden/attached/authority/strands',
    ...
    onSearch = async () => {
        await hanapin(this.state.subEntity).then(data => {
            this.setState({ datas: data }, () => console.log(this.state.datas))
        })
        ...
        await listahan(this.state.entity, this.state.datas.stages).then((data) => {
            this.setState({ strands: [...data] })
        })
    }
    newExhibit = async () => {
        this.state.strands.forEach(model => {
            let { options } = this.state
            options[model.acronym] = model.acronym
            this.setState({ options })
        });
        let strands = this.state.options
        await Swal.fire({
            input: 'select',
            inputOptions: { 'strands': strands },
            ...
            inputValidator: (value) => {
                return new Promise((resolve) => {
                    if (value) {
                        ...
                        resolve('You need to select strands :)')
                    }
                })
            }
        })
    ...
    onDelete = async (i, pk) => {
        ...
        datas.stages.shs.splice(datas.stages.shs.indexOf(pk), 1)
        this.setState({ datas })
