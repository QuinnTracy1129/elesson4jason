...
export default class extends Component {
    ...
    onSearch = (key) => {
        const params = { user: this.state.auth.id, key: key }
        ...
        let writer = this.state.models.map((model, index) => {
            return (
                <tr key={model.id}>
                    <td>{index + 1}</td>
                    <td>{model.batch}</td>
                    <td>{getStages(model.stage)}</td>
                    <td>{getSem(model.semester)}</td>
                    <td>{model.items}</td>
                    <td style={{ textAlign: "center" }}>{model.start_at}</td>
                    <td style={{ textAlign: "center" }}>{model.end_at}</td>
                    ...
                    <MDBTable striped hover responsive>
                        <MDBTableHead>
                            <tr>
                                ...
                                <th colSpan="2" style={{ textAlign: "center" }}>Exam</th>
